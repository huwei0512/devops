...
class User(object):
    ...
    def validate_login(cls,username,password):
        ...
        _count,_rt_tuple = MySQLConnection.execute_sql(_sql,(username,password_md5))
    ...
    def add(cls,username,password,age):
        password_md5 = cls.md5_str(password)
        _sql = 'insert into user_auth(username,password,age) values(%s,%s,%s)'
        args = (username,password_md5,age)
    ...
    def update(cls,uid,username,password,age):
        password_md5 = cls.md5_str(password)
        _sql = 'update user_auth set username=%s,password=%s,age=%s where id=%s'
        _args = (username,password_md5,age,uid)
